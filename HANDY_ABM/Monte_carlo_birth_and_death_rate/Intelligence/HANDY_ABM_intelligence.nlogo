...
globals[
  ...
  nominal-birth-rate-commoners            ;; nominal birth rate in HANDY set to 3%
  nominal-birth-rate-elites            ;; nominal birth rate in HANDY set to 3%
  nominal-death-rate            ;; nominal death rate in HANDY is set to 1%
  famine-max-death-rate         ;; maximum death rate in HANDY famine is 7%
  death-rate                    ;; for plotting
  flag                          ;; used for orienting the agent to best possible patch with maximum nature
]
...
;;-------------------------------- Some of the globals are soft-coded because we don't want the users of model to mess up with them --------------------------------------------------
    set nominal-birth-rate-commoners  3  ;; nominal birth rate in HANDY set to 3%
    set nominal-birth-rate-elites     3
    set nominal-death-rate  1            ;; nominal death rate in HANDY is set to 1%
    set famine-max-death-rate 7          ;; maximum death rate in HANDY famine is 7%
...
to go
 ...
 set wealth-threshold (count elites * inequality-factor * min-consumption) + (count commoners * min-consumption)

 ;;in case everybody dies then wealth-threshold becomes zero and will throw an error in calculation of famine-check. That's why we have to set the variable to zero when everyone dies.
 ifelse wealth-threshold != 0
  [set famine-check wealth / wealth-threshold]
  [set famine-check 0]

 ifelse (famine-check > 1 or famine-check = 1)
 [no-famine-rules-commoners] ;print "no famine"]
 [famine-rules-commoners] ;print "famine"]
...
tick-advance 0.1
export-plot "population with time" "population_intelligence.csv"
export-plot "Wealth" "wealth_intelligence.csv"
export-plot "Nature with time" "nature_intelligence.csv"
end
...
; instead of random wiggling, with interlligence commoners orient themselves to the best possible patch next to them in thier neighboring 8 patches.
to intelligent-movement
  set flag max[nature-amount] of patches in-radius radius
  move-to one-of patches with [nature-amount = flag] in-radius radius
  ;; old code
  ;ask patches in-radius 3
  ;[set flag max[nature-amount] of patches]
  ;set flag max[nature-amount] of neighbors
  ;ifelse (nature-amount = flag)
  ;[move-to one-of patches]
  ;[fd 0]
  ;move-to one-of patches with [nature-amount = flag]
end
...
to handy-birth-commoners
    if count commoners > 0 and random 100 < nominal-birth-rate-commoners
    [hatch 1]
end

to handy-death-commoners
    if count commoners > 0 and random 100 < nominal-death-rate
    [die]
end

to handy-birth-elites
    if count elites > 0 and random 100 < nominal-birth-rate-elites
    [hatch 1]
end

to handy-death-elites
    if count elites > 0 and random 100 < nominal-death-rate
    [die]
end

;; ===========================================  NO FAMINE CODES ============================================
to no-famine-rules-commoners
  ask commoners[
    handy-birth-commoners
    ifelse Intelligence           ;; wiggle being dumb and intelligent-movement with intelligence switch on.
    [intelligent-movement]
    [wiggle]

    extract-nature
    pay-commoners-salary
    handy-death-commoners
  ]
  ask elites[
    handy-birth-elites
    wiggle                        ;; they are still dumb because they do not extract nature.
    pay-elites-salary
    handy-death-elites
  ]
end
...
to famine-rules-commoners

  ifelse (inequality-factor = 1 or inequality-factor = 0)
  [equitable-famine]
  [unequal-famine]
...
to equitable-famine
  ask commoners[
    ...
    handy-famine-death-commoner]

  ask elites[
      handy-birth-elites
      wiggle
      pay-elites-salary-famine
      handy-famine-death-elites]
end
...
to unequal-famine
  ...
  ifelse famine-check < (1 / inequality-factor)     ;; the famine for elites only happen when famine-check goes below 1/ineqality-factor in unequal society.
  ...
  [ask elites[
    handy-birth-elites
    wiggle
    pay-elites-salary-famine                    ;; consumption decreases after w/wth < 1  (this should work)
    handy-death-elites]]

end


to handy-famine-death-commoner
  if count commoners > 0
  [let num ((1 - famine-check ) * (famine-max-death-rate - nominal-death-rate) + nominal-death-rate)
    let numr round(num)
    if random 100 < numr
    [die]
  ]
...
to pay-commoners-salary-famine
  if wealth > 0
  [set wealth wealth - famine-check * sustenance-cost]                                                                            ;; decrement the acumulated wealth
end

to pay-elites-salary-famine
  if wealth > 0
  [set wealth wealth - (famine-check * inequality-factor * sustenance-cost)]                                                      ;; decrement the acumulated wealth
end
...
to-report death-rate-commoners-monitor
    ifelse (famine-check > 1 or famine-check = 0)
  [set death-rate nominal-death-rate]
  [set death-rate round(((1 - famine-check ) * (famine-max-death-rate - nominal-death-rate) + nominal-death-rate))]
  report death-rate
end

to-report death-rate-elites-monitor
    ifelse (famine-check < 1 / inequality-factor)
  [set death-rate nominal-death-rate]
  [set death-rate round(((1 - famine-check ) * (famine-max-death-rate - nominal-death-rate) + nominal-death-rate))]
  report death-rate
end
...
"" ""
PENS
"nature" 1.0 0 -16777216 true "" "plot sum[nature-amount] of patches"
...
1495
429
Death Rate Commoners
death-rate-commoners-monitor
1
1
11
